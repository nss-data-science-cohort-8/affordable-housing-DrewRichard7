...
```{r}
st_crs(park_rra)
st_crs(sales_and_details)
```

```{r}
park_rra_projected <- st_transform(park_rra, 2274)
sales_projected <- st_transform(sales_and_details, 2274)

# bounds
park_geo <- park_rra[['geometry']]
inner_buffer <- st_buffer(park_geo, dist = 2000) # 2,000 ft buffer
outer_buffer <- st_buffer(park_geo, dist = 3000) # 3,000 ft buffer

# Create the doughnut (annular buffer)
doughnut_buffer <- st_difference(outer_buffer, inner_buffer)
# Step 4: Filter points in sales_and_details within the "doughnut" buffer
sales_and_details |> 
  filter(st_intersects(., doughnut_buffer, sparse = FALSE))
```
